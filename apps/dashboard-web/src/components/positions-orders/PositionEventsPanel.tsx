...
function eventTime(event: PositionEventLike) {
  return event.event_timestamp ?? event.created_at ?? event.timestamp ?? event.recorded_at ?? null;
}
...
function compactDetails(event: PositionEventLike) {
  const details = event.details_json ?? event.details ?? event.raw_details;
...
export default function PositionEventsPanel({ lifecycles }: Props) {
  const flattened: PositionEventLike[] = lifecycles.flatMap((item) => {
    ...
    return events.map((event): PositionEventLike => ({
      ...event,
      _position_id: event.position_id ?? positionId,
      _symbol: event.symbol ?? event.details_json?.symbol ?? event.details?.symbol ?? symbol,
    }));
  });
      ...
      {events.length === 0 ? (
        ...
        <div className="space-y-3">
          {events.map((event, idx) => {
            ...
            const key = `${event._position_id}-${event.position_event_id ?? event.event_timestamp ?? idx}`;
