...
STRATEGY_ANALYTICS_PAGE_V2_VERSION = "phase7_step13_strategy_analytics_page_v2_hotfix13_v2_only"

def _safe_get(source: str, path: str, params: dict[str, Any] | None = None, timeout: int = 30):
    payload, status_code, error = get_json(f"{EVALUATOR_BASE_URL}{path}", params=params or {}, timeout=timeout)
    if error:
...
def _service_block(payload: dict[str, Any] | None, error: dict[str, Any] | None) -> dict[str, Any]:
    return {"ok": error is None, "data": payload if error is None else None, "error": error}
...
def _section(payload: dict[str, Any] | None, key: str) -> dict[str, Any]:
    ...
    if isinstance(value, dict):
        return {"summary": value.get("summary"), "items": value.get("items") if isinstance(value.get("items"), list) else []}
    return {"summary": None, "items": []}

def get_strategy_analytics_page_v2(account_id: int, limit: int = 500, cycle_limit: int = 100) -> dict[str, Any]:
    errors: list[dict[str, Any]] = []
    v2_payload, v2_error = _safe_get("strategy_analytics_v2", "/strategy-analytics/v2", {"account_id": account_id, "limit": limit, "cycle_limit": cycle_limit}, timeout=30)
    if v2_error:
    ...
    return {
        ...
        "services": {"strategy_analytics_v2": _service_block(v2_payload, v2_error)},
        "errors": errors,
    }
